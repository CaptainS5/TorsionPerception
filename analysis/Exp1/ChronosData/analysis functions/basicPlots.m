...
if averagedPlots==1
    ...
    for eye = 1:size(eyeName, 2)
        ...
        for tS = 1:size(conditions, 2)
            tempI = find(conData.rotationSpeed==conditions(tS) & conData.eye==eyeN & conData.afterReversalD==0);
            if ~isempty(tempI)
                scatter(conData.torsionVelTMean(tempI, 1), conData.perceptualErrorMean(tempI, 1), 'LineWidth', 2)%,...
%                     'MarkerEdgeColor', markerC(tS, :), 'MarkerFaceColor', 'none')
                hold on
            end
        end
...
% saveas(gca, ['torsionPositionVSperceptPartialCorr_' endName '.pdf'])
        ...
        for eye = 1:size(eyeName, 2)
            ...
            if merged==0
                for mN = 1:2
                    tempI = find(conData.eye==eyeN & conData.afterReversalD==direction(mN)); % merged initial direction
                    dataT = conData(tempI, :);
        ...
        saveas(gca, ['meanSpeedPercept_' mergeName '.pdf'])
        ...
        saveas(gca, ['meanSpeedTorsionA_' endName '_' mergeName '.pdf'])
