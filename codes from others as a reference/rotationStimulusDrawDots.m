...
if(~parameter.singleDir)
    conditions.translationalDirection = trialConditions(3);
else
    if(parameter.moveRight == 1)
        conditions.translationalDirection = 2;
    else
        conditions.translationalDirection = 1;
    end
end
...
% % display info
% disp(['horz pix/s: ' num2str(circle.horizontalSpeedInPixel)])
% disp(['display width cm: ' num2str(display.screenWidth)])
% disp(['display distance: ' num2str(display.screenDistance)])
% disp(['display width pix: ' num2str(display.widthInPixel)])
% disp(['display.screenWidthInDegree: ' num2str(display.screenWidthInDegree)])
...
if(conditions.rotationalDirection == 1 && parameter.singleDir == 0)
    circle.rotationSpeed = circle.rotationSpeed * (-1);
    rotationalDirection = 1;
end


circle.rotationSpeed = circle.rotationSpeed * conditions.rotationalDirection;
...
%random duration fixation
%fixation.duration = rand(1)*0.5+1; %Calculate random time in seconds

%fixed duration fixation
fixation.duration = 1; 
fixation.totalFrames = round(seconds2frames(display.frameRate, fixation.duration)); %Convert seconds in frames
...
%set cue presentation mode if selected
cuePresentation = parameter.cue;
%load rotation cues if set 
if cuePresentation 
    if circle.rotationSpeed < 0
        img = parameter.ccw;
    elseif circle.rotationSpeed == 0
        img = parameter.none;
    else
        img = parameter.cw;
    end
...
fixation.pause = round(seconds2frames(display.frameRate, 0.05));

fixation.recordedTime = round(seconds2frames(display.frameRate, 0.5));

imageTexture = Screen('MakeTexture', display.windowPointer, img);
...
for frame = 1:fixation.totalFrames-fixation.pause
    ...
    if parameter.cue
        Screen('DrawTexture', display.windowPointer, imageTexture, [], [dots.center(1)-30 dots.center(2)-30 dots.center(1)+30 dots.center(2)+30]);
        Screen('DrawLine', display.windowPointer, [0 0 0],...
            dots.center(1)-halfDegree/10, dots.center(2),...
            dots.center(1)+halfDegree/10, dots.center(2), 2);
        Screen('DrawLine', display.windowPointer, [0 0 0],...
            dots.center(1), dots.center(2)-halfDegree/10,...
            dots.center(1), dots.center(2)+halfDegree/10, 2);
    else
    ...
    if(frame == fixation.totalFrames-fixation.recordedTime+1)
        tic
...
trial.stimulusFrames = round(seconds2frames(display.frameRate, trial.duration));

% CODE TO REOCRD SCREEN - FROM JOLANDE
movieWidthX = 800;%860;
movieWidthY = 800;%860;
%moviePtr = Screen('CreateMovie', display.windowPointer, 'test.mov', movieWidthX, movieWidthY, 50);


%Trial interval
for frame = 1:trial.stimulusFrames
...
pauseFrames = round(seconds2frames(display.frameRate, pauseAfterStimulus));
